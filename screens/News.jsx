...
const News = () => {
  ...
  useEffect(() => {
    ...
    getData(`https://web-production-93c3.up.railway.app/api/categories/${currentIndx}`)
    .then((data) => {
        // console.log(data);
    ...
    <SafeAreaView className="flex-1 justify-start bg-black pt-12">
      ...
      horizontal>
          {categories.map((item, index) => (
            <TouchableOpacity
            key={index}
              onPress={() => setCurrentIndx(categories[index].id - 1)}
              style={{
                backgroundColor: categories[index].id - 1 === currentIndx ? "#ffffff" : null,
              }}
            ...
            >
              <View className="flex-row items-center">
                ...
                <Text
                  style={{
                    color: categories[index].id - 1 === currentIndx ? "#000000" : "#ffffff",
      ...
      {/* Page Title */}
      <View style={{ marginTop: -160 }}  className="w-[90%] flex-row items-center mb-6 ml-6">
        <Image
          source={images.Pop_corn}
          className="w-[24px] h-[24px] mr-3"
          resizeMode="contain"
